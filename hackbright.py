...
# print """Welcome to the giant Hackbright database. Here's how to add things.
# Need to add a new student? Type "new_student" and then the student's first name, last name, and github ID.
# Need to find a student's name? Type "student" and then the student's github ID.
#         """ 
...
def assign_grade(github, title, grade):
    ...
    row = db_cursor.fetchone()
    db_connection.commit()
    return "Github user %s received a %s on project %s." % (row[0], row[2], row[1])
...
def handle_input():
    ...
    while command != "quit":
        ...
        if command == "student":
            if len(args) == 1:
                github = args[0]
                returned_github_id = get_student_by_github(github)
                return returned_github_id
            else:
                return "Too many arguments, please provide only github username you are seeking"
            ...
            if len(args) == 3:
                ...
                return "Need all student info - first name, last name and github username."
